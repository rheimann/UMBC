...
colnames(BB) <- c("location", "year", "month", "day", "url", "text", "date")
colnames(BB)
# [1] "text" "year" “date"
...
BB.score<- score.sentiment(BB$text, pos.words, neg.words, .progress = 'text')
...
BB.boxplot<- BB.boxplot + xlab("Year") + ylab("Sentiment (Centered)") +
  ggtitle("Economic Sentiment - Beige Book (1996-2010)")
# draw boxplot
BB.boxplot
...
# dallas <- subset(BB.sentiment, location=="Dallas")bb.tbl <- ddply(BB.sentiment, "BB.sentiment$location", numcolwise(sum))
# colnames(bb.tbl)
# colnames(bb.tbl) <- c("location", "year", "score", "mean", "centered", "pos", "neg")
# bb.tbl <- bb.tbl[ , c("location", "score", "centered", "pos", "neg")]
# bb.tbl <- bb.tbl[order(bb.tbl$score),]
# table(bb.tbl$location, bb.tbl$high)
# bb.tbl <- as.table(bb.tbl)
